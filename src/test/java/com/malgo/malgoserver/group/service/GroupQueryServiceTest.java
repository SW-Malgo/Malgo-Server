...
class GroupQueryServiceTest {
	...
	@Autowired GroupQueryService groupQueryService;
	@Autowired KeywordRepository keywordRepository;
	@Autowired GroupRepository groupRepository;
	@Autowired KeywordGroupRepository keywordGroupRepository;

	@Test
	public void queryGroup() {

		Keyword keyword1 = Keyword.builder().tag("운동").build();
		Keyword newKeyword1 = keywordRepository.save(keyword1);

		Group group1 = Group.builder().name("운동그룹").groupContent("asd").max_count(2l).build();
		Group newGroup1 = groupRepository.save(group1);

		KeywordGroup keywordGroup1 =
				KeywordGroup.builder().keyword(newKeyword1).group(newGroup1).build();
		keywordGroupRepository.save(keywordGroup1);

		Keyword keyword2 = Keyword.builder().tag("게임").build();
		Keyword newKeyword2 = keywordRepository.save(keyword2);

		Group group2 = Group.builder().name("게임그룹").groupContent("asd").max_count(2l).build();
		Group newGroup2 = groupRepository.save(group2);

		KeywordGroup keywordGroup2 =
				KeywordGroup.builder().keyword(newKeyword2).group(newGroup2).build();
		keywordGroupRepository.save(keywordGroup2);

		List<Long> keywords = Arrays.asList(newKeyword1.getId());
