...
public class GroupServiceFacade {
    ...
    public List<GroupResponse> execute(List<Long> keywords, Boolean withKeyword) {
        ...
        List<GroupImage> collect = groups.stream().map(g -> groupImageRepository.findByGroup(g).orElseThrow(() -> new IllegalArgumentException())).collect(Collectors.toList());
        List<List<KeywordGroup>> collect1 = groups.stream().map(g -> keywordGroupRepository.findAllByGroup(g)).collect(Collectors.toList());
        List<List<Keyword>> collect2 = collect1.stream().map(lk -> lk.stream().map(k -> keywordRepository.findById(k.getId())).collect(Collectors.toList())).collect(Collectors.toList());

        List<GroupResponse> groupResponses = new ArrayList<>();
        for (int i = 0; i < groups.size(); i++) {
            groupResponses.add(GroupResponse.builder().groupThumbnail(collect.get(i).getSource()).groupName(groups.get(i).getName()).groupTag(collect2.get(i)).groupContent(groups.get(i).getGroupContent()).build());
        }
